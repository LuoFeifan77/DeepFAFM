...
class BaseModel:
    ...
    def __init__(self, opt):
        ...
        # else:
        #     self.device = torch.device('cuda', opt['num_gpu'])
        ...
        # set gpu index
        self.opt = opt
        self.device = torch.device("cuda:" + str(self.opt['gpu_idx']))  # 指定下标的gpu运行
        self.is_train = opt['is_train']
    ...
    def validation(self, dataloader, tb_logger, update=True):
        ...
        for index, data in enumerate(pbar):
            p2p, Pyx, Cxy, gs_x, gs_y = self.validate_single(data, timer)  # 保存滤波器看下
            p2p, Pyx, Cxy, gs_x, gs_y = to_numpy(p2p), to_numpy(Pyx), to_numpy(Cxy), to_numpy(gs_x), to_numpy(gs_y)


            # save_filter_x = {'gs_x': gs_x}  # 保存好了 matlab绘制！
            # sio.savemat(os.path.join(self.opt['path']['visualization'], 'filter_x.mat'), save_filter_x) # save filter

            # save_filter_y = {'gs_y': gs_y}  # x和y都要保存
            # sio.savemat(os.path.join(self.opt['path']['visualization'], 'filter_y.mat'), save_filter_y) # save filter

            # data_x, data_y = data['first'], data['second']
            # name_x, name_y = data['first']['name'][0], data['second']['name'][0]

            # save_dict = {'Cxy_gt': Cxy_gt, 'Cxy': Cxy, 'p2p': p2p + 1, 'gs_x': gs_x, 'gs_y': gs_y}  # plus one for MATLAB
        ...
        # visualize results
            if self.opt.get('visualize', False):
                ...
                sio.savemat(os.path.join(self.opt['path']['visualization'], f'{name_x}-{name_y}.mat'), save_dict) # save matching results

                # save the geodesic of each matching pair
                # logger.info(f'Start training from epoch: {model.curr_epoch}, iter: {model.curr_iter}')  # 这些信息怎么记录的
                logger = get_root_logger()  
                logger.info(f'{name_x}-{name_y}, geo_err: {geo_err.mean():.4f}') # 看看是不是这样打印的

        # save learned filters gs_x, gs_y
        save_filter_x = {'gs_x': gs_x}
        sio.savemat(os.path.join(self.opt['path']['visualization'], 'filter_x.mat'), save_filter_x) # save filter

        save_filter_y = {'gs_y': gs_y}
        sio.savemat(os.path.join(self.opt['path']['visualization'], 'filter_y.mat'), save_filter_y) # save filter
    ...
    def _networks_to_device(self):
        ...
        for name, network in self.networks.items():
            ...
            # self.device = torch.device("cuda:" + str(self.opt['gpu_idx']))
            ...
            #         network = DataParallel(network, device_ids=list(range(self.opt['num_gpu']-1, -1, -1)))
            #     else:
            #         raise ValueError(f'Invalid backend: {self.opt["backend"]}, only supports "dp", "ddp"')
                
            # network = DataParallel(network, device_ids=[self.opt['gpu_idx']])  
