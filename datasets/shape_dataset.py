...
class SingleFaustDataset(SingleShapeDataset):
    def __init__(self, data_root,
        ...
        if phase == 'train':
            ...
            self._size = 80
        elif phase == 'test':  # last 20
            if self.off_files:
                self.off_files = self.off_files[-20:]
            if self.corr_files:
                self.corr_files = self.corr_files[-20:]
            if self.dist_files:
                self.dist_files = self.dist_files[-20:]
            self._size = 20
        ...
        with open(os.path.join(self.data_root, f'{self.phase}.txt'), 'r') as f:
            ...
            for line in lines:
                ...
                if line.split('/')[0] not in self.ignored_categories:
                    self.off_files += [os.path.join(self.data_root, 'off', f'{line}.off')]
                    if self.return_corr:
                        self.corr_files += [os.path.join(self.data_root, 'corres', f'{line}.vts')] #加载的对应
                    if self.return_dist:
                        self.dist_files += [os.path.join(self.data_root, 'dist', f'{line}.mat')]
