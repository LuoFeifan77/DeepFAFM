...
class SquaredFrobeniusLoss(nn.Module):
    ...
    def forward(self, a, b):
        loss = torch.sum(torch.abs(a - b) ** 2, dim=(-2, -1))  # 这里的mean 有用吗？
...
class SURFMNetLoss(nn.Module):
    ...
    def __init__(self, w_bij=1.0, w_orth=1.0, w_lap=1e-3, bidirectional=True):
        """
        Init SURFMNetLoss
    ...
    def forward(self, C12, C21, evals_1, evals_2):
        ...
        if self.w_orth > 0:
            ...
            if self.bidirectional:
                orthogonality_loss +=  criterion(torch.bmm(C21.transpose(1, 2), C21), eye_batch)
